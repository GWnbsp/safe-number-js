import { SafeNumberOptions, ProcessResult, RoundingMethod } from './types';
...
class SafeNumber {
    ...
    private validateOptions(): void {
        ...
        if (typeof this.options.decimalPlaces !== 'number' || 
            this.options.decimalPlaces < 0) {
            throw new Error('decimalPlaces must be a non-negative number');
        }
    }

    public handleResponse(data: any): ProcessResult {
        try {
            const processedData = this.recursiveHandle(data);
            return {
                success: true,
                data: processedData
            };
    ...
    private recursiveHandle(data: any, currentKey: string = ''): any {
        if (Array.isArray(data)) {
            return data.map(item => this.recursiveHandle(item, currentKey));
        }
        ...
        return this.handleValue(data, currentKey);
    }
    ...
    private handleValue(value: any, key: string): any {
    ...
    public customRound(value: number, precision: number, method: RoundingMethod = 'round'): number {
    ...
    public formatCurrency(value: number, currency: string = 'CNY'): string {
    ...
    public batchProcess(items: any[]): ProcessResult {
        try {
